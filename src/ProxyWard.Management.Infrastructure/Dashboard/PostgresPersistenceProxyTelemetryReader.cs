...
public sealed class PostgresPersistenceProxyTelemetryReader : IProxyTelemetryReader, IAsyncDisposable, IDisposable
{
    ...
    public PostgresPersistenceProxyTelemetryReader(string connectionString, ManagementAuditReadOptions options)
        : this(CreateDataSource(connectionString), options, ownsDataSource: true)
    {
    }

    public PostgresPersistenceProxyTelemetryReader(NpgsqlDataSource dataSource, ManagementAuditReadOptions options)
        : this(dataSource, options, ownsDataSource: false)
    {
    }

    private PostgresPersistenceProxyTelemetryReader(
        ...
        CancellationToken cancellationToken)
    {
        ...
        var counts = await ReadAggregateCountsAsync(connection, transaction, fromIso, toIso, cancellationToken)
        ...
        if (counts.TotalCount > 0 && counts.AsOfUtc.HasValue)
        {
            ...
            {
                var hasOlderRows = await ReadHasOlderRowsAsync(connection, transaction, fromIso, cancellationToken)
                    .ConfigureAwait(false);
    ...
    private void ThrowIfDisposed()
    {
        if (_disposed)
        {
            throw new ObjectDisposedException(nameof(PostgresPersistenceProxyTelemetryReader));
        }
    }
